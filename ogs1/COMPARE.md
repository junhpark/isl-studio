...
정호 패턴식 · 포화 피압대수층 · 보존성 추적자 · 2026-09-19 (부록 B 정정 2026-09-22)

## 한 줄 결론

브라우저 근사 모델(ISL Studio 패턴 탭)은 같은 시나리오의 OGS-6 참조해와 **수두에서 1~3 %** 이내로 일치한다. 도메인을 맞춘 12개 시나리오(부록 B)에서 sweep 접촉율은 브라우저가 **평균 1.5 %p 낮고**(1 PV 기준 −5~+0.4 %p; 봉쇄가 유지되는 bleed +5 % 조건에서 −2~−5 %p, 봉쇄가 깨진 bleed −10 % 조건에서 ±1 %p), 광체 밖 유출 분율은 봉쇄 조건에서 **±0.6 %p**, 봉쇄가 깨진 조건에서 브라우저가 **2~3 %p 높다**. 회수정 파과 시각은 −3~+6일. 브라우저의 유실 수치를 "상한"으로 읽어야 한다고 했던 1일차 결론은 **철회**한다. 그 결론은 결함 있는 참조해에서 나온 것이었다.

> **정정 (2026-09-22).** 부록 A 까지의 12개 라이브러리 수치는 참조해 도메인이 288 m 로 고정된 채 계산된 것으로, 12개 중 10개가 스튜디오와 다른 도메인이었다. 다시 계산한 결과는 부록 B 에 있고, 위 결론은 부록 B 기준이다. 부록 A 의 표는 기록으로 남긴다.
...
- **sweep 접촉율** — 분산항 도입 전 평균 3 %p 낮았고, 도입 후 평균 1.5 %p 낮다(부록 B). 봉쇄된 조업(bleed +5 %)에서 2~5 %p 낮고, 봉쇄가 깨진 조업에서는 ±1 %p. 브라우저의 sweep 을 약간 보수적인 값으로 읽는다.
- **광체 밖 유실 분율** — 봉쇄된 조업에서는 참조해와 ±0.6 %p. 봉쇄가 깨진 조업에서는 브라우저가 2~3 %p 크게 낸다(그때만 상한으로 읽는다).
- **파과 시각** — −3~+6일. 운영 일정 논의에는 무시해도 되는 차이.
...
OMP_NUM_THREADS=1 python runner/make_library.py --worker 0/2 &   # 12 케이스, 2코어 약 40분 (1/2 도 함께)

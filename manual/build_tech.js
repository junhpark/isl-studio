...
body.push(B('도메인은 정호 패턴 외곽에서 감시 링 거리(25 m) + 44 m를 더한 정사각형이며, 항상 80 × 80 셀로 나눈다. 기본 5-spot 50 m에서는 288 m, Δx = 3.6 m.'));
...
body.push(P('이 보정은 등속 1차원 결과를 2차원 비균일 속도장에 적용한 것이므로 근사입니다. 정호 근처처럼 속도가 급변하는 곳에서는 수치분산이 국소적으로 Δx/2와 다르고, 그 잔차가 참조해와의 남은 차이(sweep −0.6 %p, 파과 수 일)에 기여합니다.'));
...
body.push(table(['구성요소', '출처 / 버전', '역할'], [
  ...
  ['numpy', 'PyPI', '결과 배열 처리, 지표 계산, fields.bin(Float32) 기록'],
...
body.push(NUM('runner/validate.py로 스키마 검증. runner/make_case.py가 정호 좌표를 스튜디오와 같은 규칙으로 생성하고, generateStructuredMesh → ExtractBoundary → identifySubdomains로 메시를 만들고, 템플릿에 값을 채워 pattern.prj를 씁니다. case_meta.json에 절점 ID와 1 PV 일수를 기록합니다.'));
body.push(NUM('runner/run_case.py가 ogs를 실행하고, 5일마다 나온 .vtu를 읽어 절점값을 브라우저 셀 순서(i + j·nx)로 재배열해 fields.bin(Float32 [시각][수두, 추적자][셀])과 results.json(시각, 정호, sweep, 광체 밖 분율, 물질수지)을 씁니다.'));
...
body.push(NUM('스튜디오에서 results.json + fields.bin을 불러오면, 격자·도메인 일치를 확인한 뒤 물질수지 결함을 경고하고, “OGS 참조해 표시”를 켜면 침출액·수두 모드가 OGS 필드를 그리며 지표에 RMS·R²·OGS sweep이 뜹니다.'));
body.push(P('헤드리스 비교(browser/make_browser_snapshots.js, library_browser.js)는 스튜디오의 패턴 모듈을 Node에서 그대로 돌려 화면 없이 스냅샷을 만듭니다. 12개 라이브러리는 runner/make_library.py로 병렬 실행합니다.'));
...
body.push(table(['지표', '브라우저', 'OGS', '차이', '해석'], [
  ['수두 RMS 차', '—', '—', '0.13 m (수두 범위의 1 %)', '흐름 해 일치. 차이는 정호 근처 셀/절점 형상'],
  ['sweep @ 1 PV', '70 %', '72 %', '−2 %p (12개 평균 −0.6 %p)', '분산 보정 후 잔차'],
  ['sweep @ 5 PV', '90 %', '91 %', '−1 %p', '장기적으로 수렴'],
  ['광체 밖 최대', '3.0 %', '3.5 %', '−0.5 %p', '봉쇄 성립 시 일치'],
  ['파과 시각', '35 d', '30 d', '+5 d (12개: −2 ~ +5 d)', '풍상차분의 전선 지연']], [1500, 1200, 1200, 2300, 2800]));
body.push(SP());
...
body.push(P('3 패턴(5-spot, 7-spot, 선형) × 2 간격(30, 50 m) × 2 bleed(+5 %, −10 %)의 12개를 같은 파이프라인으로 돌렸습니다(cases/LIBRARY_disp.md). 요약하면:'));
body.push(B('sweep @1 PV: 브라우저 − OGS 평균 −0.6 %p, 범위 −3.7 ~ +1.6 %p; @5 PV 평균 −0.5 %p. 분산 보정 전(+1.5 %p 과대)보다 개선.'));
body.push(B('광체 밖 최대: 봉쇄 성립(bleed +5 %)에서는 ±1 %p. 봉쇄 실패(bleed −10 %)에서는 브라우저가 3~5 %p 과대. 후자는 유출 플룸이 도메인 가장자리에 닿는 경우로, 브라우저의 “경계 밖 유출” 계정과 OGS의 경계 처리(고정 압력 절점에서의 이류 유출)가 달라서 생깁니다.'));
body.push(B('파과: 브라우저가 −2 ~ +5일(대체로 늦음). 간격 30 m(파과가 빠른 경우)에서 차이가 작고, 50 m에서 큽니다.'));
...
body.push(P('결론: 봉쇄가 성립하는 운영 범위에서 브라우저 근사는 참조해와 수 %p 이내로 일치하며, 정책 시연과 정성 비교 용도로 충분합니다. 봉쇄가 깨지는 시나리오에서 유출량의 절대값은 브라우저가 보수적으로(크게) 나오므로, 그 숫자 자체를 인용해서는 안 됩니다.'));

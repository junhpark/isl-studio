...
body.push(P('이 방식은 포화대가 아니라 불포화 사면에서 이루어집니다. 사면 상부에 얕은 주입공을 촘촘히 뚫어 황산암모늄 용액을 넣으면 용액은 중력으로 풍화층을 통과해 내려가다가 기반암(불투수층) 위에 고이고, 사면 아래쪽으로 흘러 하단의 집액구(collection trench)·집액 갱도로 모입니다. 압력으로 흐름을 제어할 수단이 없으므로, 침출제이 어디로 가는지는 기반암 상면의 형상과 연속성, 풍화층의 투수성 분포에 전적으로 달려 있습니다. 기반암에 균열이나 결손이 있으면 용액은 그대로 지하수로 들어가고, 이것이 이 방식의 암모니아성 질소 오염 문제의 물리적 원인입니다.'));
...
body.push(P('물에 녹은 물질(침출제(lixiviant, 주입 용액), 추적자, 희토류 이온)은 물과 함께 흘러가고(이류, advection), 공극 구조 때문에 경로별 속도가 달라 번지며(기계적 분산, mechanical dispersion), 분자 확산으로 퍼집니다. 이 셋을 합친 것이 이류–분산 방정식(ADE)입니다.'));
...
body.push(...EQ('exchange', '식 (9). 화학량론상 RE³⁺ 1 mol당 NH₄⁺ 3 mol, 즉 (NH₄)₂SO₄ 1.5 mol이 필요합니다. 코드에서 침출제 소비를 1.5·r로 잡은 근거입니다. 실제로는 Al³⁺, Ca²⁺ 등 경쟁 양이온이 훨씬 많은 암모늄을 소비하므로 시약 소비량은 이 하한보다 큽니다.'));
body.push(P('프로토타입은 교환 평형을 명시적으로 풀지 않고 단일 속도식으로 대체합니다. 남아 있는 흡착량에 비례하고(1차), 침출제 농도에 대해 포화형(Langmuir/Monod형)으로 반응하며, 불포화 셀에서는 유효포화도만큼 접촉이 줄어듭니다.'));
body.push(...EQ('kinetic', '식 (10). S: 셀의 잔존 흡착 RE [mol], c: 셀 내 침출제 농도 [mol/m³], k_r: 속도상수(기본 0.30 /d), K_H: 반포화 농도(40 mol/m³), M_l, M_r: 셀 내 침출제·용존 RE 몰수. 침출제이 K_H보다 훨씬 진하면 속도는 k_r·S로 포화되고, 묽어지면 농도에 비례합니다. 한 단계에서 r은 잔존량 S와 가용 침출제 M_l/1.5를 넘지 못하도록 잘립니다. 사면 모듈은 여기에 S_e 인자가 곱해지고, 포화 모듈(패턴식)에서는 S_e = 1입니다.'));
...
body.push(SP());
body.push(P('K 값은 Yuan et al.(2025) Table 1의 현장 측정치이고, 함수비와 강도는 풍화 화강암 규석토의 일반적 범위에서 골랐습니다. 셀 상태 변수는 물 높이 w [m, 셀 단면당 물 부피/면적], 침출제 몰수 M_l, 용존 RE 몰수 M_r, 흡착 RE 몰수 S입니다. 옵션 “기반암 차수층 결손”은 사면 중앙 아래의 기반암 셀 블록(x 16~25, y 7~16)의 K를 0.6 m/d로 올리고, “차수벽”은 최하류 두 열의 K를 10⁻⁷ m/d로 내립니다.'));
...
body.push(table(['단계', '기간 (일)', '주입', '침출제 농도'], [
...
body.push(P('식 (10)을 광석층 셀에서 명시적으로 적분합니다. 침출제 농도가 1 mol/m³ 미만이면 반응을 건너뛰고, 한 단계의 반응량 r은 min(k_r·S·c/(K_H+c)·S_e·Δt, S, M_l/1.5)입니다. 반응은 이동 전에 계산되므로 “그 단계에 셀에 있던 침출제”이 반응합니다. Δt = 0.05 d, k_r = 0.30 /d이면 단계당 최대 1.5 %만 변하므로 명시적 적분이 안정합니다.'));
...
body.push(B('밀도·점성은 일정(침출제 농도가 흐름에 영향을 주지 않음). 침출제과 RE 이온은 같은 속도장으로 이동한다.'));
...
body.push(...EQ('pv', '식 (18). PV(pore volume, 공극체적): 광체 범위의 공극 부피(기본 설정 54,000 m³). β: bleed. 총 회수량은 총 주입량의 (1+β)배이고 회수정 수로 균등 분배합니다. β < 0이면 주입이 양수를 초과해 봉쇄가 깨집니다. 화면의 시간축은 누적 양수량 ÷ PV이며, 1 PV 시간은 총 양수량으로 PV만큼 퍼내는 데 걸리는 시간입니다(기본 5-spot 50 m, β = 0.05: 약 129일/PV). 양수를 기준으로 잡는 이유는 복원 단계(주입 정지)에도 시간축이 계속 진행해야 하고, 브라우저와 OGS 참조해가 같은 시각을 가리켜야 하기 때문입니다.'));
...
body.push(...EQ('upwind', '식 (22). 침출제(C_l)과 용존 RE(C_r) 두 성분을 같은 식으로 옮깁니다. 주입정은 c_inj 농도의 물을, 회수정은 그 셀 농도의 물을 넣고 뺍니다. 도메인 가장자리에서 밖으로 나가는 유량은 “경계 유출” 계정(lsOut)에 누적합니다.'));
...
body.push(P('이 보정은 등속 1차원 결과를 2차원 비균일 속도장에 적용한 것이므로 근사입니다. 정호 근처처럼 속도가 급변하는 곳에서는 수치분산이 국소적으로 Δx/2와 다르고, 그 잔차가 참조해와의 남은 차이(sweep@1 PV 평균 −2.4 %p, 파과 수 일)에 기여합니다.'));
body.push(H2('5.7 침출 반응·회수·복원'));
body.push(P('반응은 식 (10)에서 S_e = 1로 두고 광체 셀에서만 이동 전에 계산합니다. 회수정에서 나간 용존 RE 몰수를 누적해 회수율(초기 흡착 몰수 대비)과 모액 농도를 만듭니다. 누적 회수율이 50 %, 80 %에 도달한 PV를 기록해 “몇 PV를 돌려야 하는가”를 보여줍니다. 누적 양수 PV가 침출 종료 PV(기본 5)에 도달하면 복원 단계로 들어가 주입 유량을 0으로 두고 수두를 다시 풀며, 양수는 계속되어 잔류 침출제을 뽑아냅니다(기본 2 PV). 이때 유선은 다시 추적되지만 봉쇄 지표는 침출 단계의 값을 유지합니다.'));
body.push(H2('5.8 감시 링과 UCL 규칙'));
body.push(P('광체 범위에서 25 m 바깥의 사각 링에 감시정 12공(변마다 3공)을 둡니다. 2일마다 시료를 채취해 침출제 농도가 주입 농도의 3 %(UCL)를 넘는지 보고, 2회 연속 초과하면 경보를 냅니다. 이는 미국 ISR 우라늄 규제(NRC/주정부 허가)의 “UCL 초과 → 확인 시료 → 시정조치” 절차를 단순화한 것입니다. 경보 수는 화면 상태 상자에 표시되며, 운영을 자동으로 바꾸지는 않습니다.'));
body.push(H2('5.9 지표의 정의'));
body.push(...EQ('pattern_metrics', '식 (26). sweep 접촉율: 광체 셀 중 침출제 농도가 주입 농도의 5 %를 넘는 셀의 비율(“침출제이 닿은 광석의 비율”). 유실률 L: 경계 밖으로 나간 침출제 몰수와 광체 범위 밖 셀에 있는 침출제 몰수의 합을 총 주입 몰수로 나눈 값. 8단계마다 계산해 현재값과 최대값을 유지합니다. 회수율·모액 농도·시약 소비는 사면 모듈과 같은 정의입니다.'));
...
body.push(NUM('스튜디오 목록(cases/index.json)에서 케이스를 고르거나 results.json + 필드 파일을 직접 불러오면, 필드를 풀어(gzip 해제·역양자화, 브라우저의 DecompressionStream) 격자·도메인 일치를 확인한 뒤 물질수지 결함을 경고하고, “OGS 참조해 표시”를 켜면 침출제·수두 모드가 OGS 필드를 그리며 지표에 RMS·R²·OGS sweep이 뜹니다.'));
...
body.push(P('5-spot, 간격 50 m, 규모 2×2, 외곽 회수정 9공, 주입정 4공 × 100 m³/d, bleed +5 %, 추적자만(반응 끔), α_L = 2 m, α_T = 0.2 m, 7 PV(침출 5 + 복원 2, 약 900일).'));
body.push(table(['지표', '브라우저', 'OGS', '차이', '해석'], [
  ['수두 RMS 차', '—', '—', '0.13 m (수두 범위의 1 %; 12개: 1~3 %)', '흐름 해 일치. 차이는 정호 근처 셀/절점 형상'],
  ['sweep @ 1 PV', '70 %', '72 %', '−2 %p (12개 평균 −2.4 %p)', '분산 보정 후 잔차'],
  ['sweep @ 5 PV', '90 %', '91 %', '−1 %p', '장기적으로 수렴'],
  ['광체 밖 최대', '3.0 %', '3.5 %', '−0.5 %p', '봉쇄 성립 시 일치'],
  ['파과 시각', '35 d', '30 d', '+5 d (12개: −3 ~ +5 d)', '풍상차분의 전선 지연']], [1500, 1200, 1200, 2300, 2800]));
body.push(SP());
...
body.push(P('3 패턴(5-spot, 7-spot, 선형) × 2 간격(30, 50 m) × 2 bleed(+5 %, −10 %)의 12개를 같은 파이프라인으로 침출 5 PV + 복원 2 PV까지 돌렸습니다(cases/LIBRARY.md). 도메인은 케이스마다 스튜디오 공식으로 정해 228~372 m이고, 시간축은 두 모델 모두 누적 양수량 기준 PV라 같은 PV가 같은 날짜입니다. 이전 두 차례의 비교는 도메인(288 m 고정)과 PV 기준(주입량 vs 양수량)이 어긋나 있었고, 7-spot은 외곽 고리가 주입정으로 배치돼 있었습니다. 이를 모두 바로잡은 수치가 아래입니다(COMPARE.md 부록 C). 요약하면:'));
body.push(B('수두 RMS: 5-spot 0.12~0.14 m(범위의 1.0~1.3 %), 7-spot 0.21~0.23 m(1.4~1.6 %), 선형 0.19~0.35 m(1.5~3.0 %). 정호가 촘촘한 선형 30 m에서 정호 근처 셀/절점 형상 차이가 가장 큽니다.'));
body.push(B('sweep @1 PV: 브라우저 − OGS 평균 −2.4 %p, 범위 −5.2 ~ −1.2 %p; @5 PV 평균 −0.8 %p. bleed +5 %와 −10 %에서 평균이 같아(각 −2.4 %p) 봉쇄 조건과 무관합니다. 분산 보정 전(평균 −3.9 %p)보다 40 % 줄었습니다.'));
body.push(B('광체 밖 최대(침출 중): 봉쇄 성립·실패 모두 ±0.6 %p. 이전 비교에서 봉쇄 실패 시 브라우저가 2~3 %p 과대로 보였던 것은 PV 기준 차이로 브라우저가 더 오래 돌았기 때문이었습니다.'));
body.push(B('파과: −3 ~ +5일. 5-spot과 7-spot에서는 브라우저가 늦고, 선형에서는 같거나 빠릅니다.'));
body.push(B('복원: 5 PV에서 주입을 멈추고 2 PV 양수하면 도메인 내 추적자가 5 PV 시점의 20~28 %로 줄며, 두 모델이 ±2 %p 안에서 같습니다(브라우저가 평균 0.8 %p 더 빨리 빼냄).'));
...
body.push(P('결론: 브라우저 근사는 참조해와 sweep에서 1 PV 기준 약 2 %p 낮게(보수적), 유출에서 ±0.6 %p, 복원 잔류에서 ±2 %p 안에서 일치하며 이 차이는 bleed 조건에 따라 달라지지 않습니다. 정책 시연과 정성 비교 용도로 충분합니다. 다만 근사 모델이므로 어느 숫자든 참조해 없이 단독으로 인용해서는 안 됩니다.'));
...
body.push(table(['기호', '의미', '단위', '기본값'], [
  ...
  ['S, M_l, M_r', '셀 흡착 RE, 침출제, 용존 RE 몰수', 'mol', '—'],

...
body.push(BR([{ text: '정호 패턴식 — ', bold: true }, { text: '우라늄·구리 ISR에서 쓰는 방식. 포화 피압대수층에 주입정과 회수정을 격자로 배치하고, 양수량을 주입량보다 몇 % 많게(bleed) 유지해 침출제을 수리학적으로 가둡니다.' }]));
...
body.push(NUM('사면 중력식 탭에서 하단의 재생 버튼(▶)을 누르거나 스페이스바를 누릅니다. 속도는 3×가 적당합니다. 침출제(시안색)이 표토를 지나 기반암 위에 고이고 사면 아래로 흐르는 것을 봅니다.'));
...
body.push(table(['영역', '위치', '내용'], [
  ['앱 바', '최상단', '로고, 탭(사면 중력식 / 정호 패턴식 / 비교), KO/EN 언어 전환, PROTOTYPE 배지와 ? 버튼(누르면 안내 창)'],
  ...
  ['상태 상자', '3D 우상단', '경과 시간(일 또는 PV — 옆의 ⓘ에 시간축 설명), 현재 단계와 구간(침출 0–5 PV, 복원 5–7 PV), 지금 일어나는 일의 한 줄 설명. 3D 면에 OGS 결과를 켜면 그 상태를 알리는 줄이 추가됨'],
  ['범례', '3D 우하단', '면(색)과 선·표식. 제목줄을 누르면 접힘. 항목 이름에 마우스를 올리면 상세 설명'],
  ['지표 패널', '우측', '핵심 지표 3개(정상·주의·위험 표시), 경고, 시간 이력 차트(위: 회수율·유실률 %, 아래: 안전율 또는 모액 농도), 참조해 비교(참조해를 불러오면 나타남), 상세 지표·모델 한계(접힘)'],
  ['타임라인', '최하단', '재생/일시정지, 초기화, 진행 막대, 속도 1×·3×·10×, 절개 슬라이더'],
], [1500, 1400, 6200]));
body.push(SP());
body.push(P('화면 문구는 한국어와 영어 두 벌입니다. 앱 바 우측의 KO/EN 버튼으로 바꾸며, 선택은 브라우저에 기억됩니다. 주소에 ?lang=en을 붙여 열면 처음부터 영어로 열립니다. 이 설명서와 기술 배경서는 한국어판만 있습니다.'));
...
body.push(P('100 × 60 m 사면 도메인을 2.5 m 격자(40 × 24 × 28 셀)로 나눕니다. 표토(1.5 m), 전풍화층(광체, 10 m), 반풍화층(6.5 m), 기반암(자연 차수층)의 4개 층이 지형을 따라 놓입니다. 투수계수는 Yuan et al. (2025) Table 1의 7.08 / 2.59 / 1.30 / 0.00038 m/d를 씁니다. 표토와 기반암 사이에 네 자릿수 차이가 있어 침출제은 표토를 순식간에 지나 기반암 위에서 멈춥니다.'));
...
body.push(table(['항목', '색', '무엇을 보는가'], [
  ['침출제', '시안', '주입한 황산암모늄 용액(lixiviant) 농도. 용액이 어디까지 갔는지. 기본값'],
  ['모액', '황 → 적', '용액 중 희토류 농도. 집액구로 오는 회수 대상'],
  ['포화도', '남색', '침출제 포화도. 정체수가 어디 고이는지'],
...
body.push(P('두께 8 m의 포화 피압대수층을 80 × 80 평면 격자로 풉니다. 수두는 정호 조건이 바뀔 때만 Laplace 방정식을 다시 풀고(SOR), 그 속도장으로 침출제과 회수 대상을 이류·분산시킵니다. 위아래 차수층은 흐름에 참여하지 않습니다. 도메인 크기는 정호군 크기에 맞춰 자동으로 정해집니다(기본 288 m).'));
body.push(P('시간은 일이 아니라 PV(pore volume, 공극체적)입니다. PV는 광체 범위의 공극 부피(면적 × 두께 × 공극률, 기본 설정 54,000 m³)이고, 시간축은 누적 양수량을 PV로 나눈 값, 즉 광체 속 물을 몇 번 갈았는가입니다. 우라늄 ISR 실무 단위이고, 정호 간격이 달라도 같은 기준으로 비교할 수 있습니다. 상태 상자의 PV 옆 ⓘ에 이 설명이 있습니다. 0–5 PV 침출(주입 + 양수), 5–7 PV 복원(주입 정지, 양수 지속). 1 PV의 일수는 양수량 기준이라 bleed에 따라 조금 달라집니다(기본 설정 129일).'));
...
body.push(table(['항목', '범위 · 기본값', '뜻'], [
  ...
  ['외곽 정호 = 회수정', '켜기 · 기본 켜짐', '켜면 가장 바깥 고리가 회수정(Florence Copper 방식 · 5-spot 2×2에서 주입 4 : 회수 9, 7-spot 2×2에서 6 : 7). 끄면 주입정이 바깥'],
  ['주입량 / 정호', '30–300 · 100 m³/d', '투수계수가 낮으면 허용 수위강하(15 m) 안에서 자동 제한됨'],
  ['Bleed', '−15 – +15 · +5 %', '양수가 주입보다 많으면 안쪽으로 동수경사가 생겨 침출제이 갇힘. 음수면 밖으로 밀려남. IAEA NF-T-1.4: 수 %'],
...
body.push(table(['항목', '무엇을 보는가'], [
  ['침출제', '주입 용액(lixiviant)의 추적자 농도. 플룸이 어디까지 갔는지. 기본값'],
  ['모액', 'PLS(pregnant leach solution). 용액 중 회수 대상 농도'],
  ['침출 진행도', 'sweep. 정호 사이에 침출제이 닿지 않는 꽃잎 모양 정체 구역이 남는 것이 회수 한계의 이유'],
...
body.push(BR([{ text: '유실률 — ', bold: true }, { text: '현재 도메인 내 침출제 중 광체 범위 밖에 있는 비율. 복원 양수로 되끌려오면 줄어듭니다. 상세 지표의 “유실률 최대(침출 중)”가 조업 중 최악값입니다.' }]));
body.push(BR([{ text: '감시정 초과 — ', bold: true }, { text: '12공 중 알람 상태인 수.' }]));
body.push(BR([{ text: '1 PV 소요 — ', bold: true }, { text: '상세 지표. 투수계수를 낮추면 정호당 주입량이 제한되어 이 값이 급격히 늘어납니다. 이온흡착형 가정(0.16 m/d)에서 129 → 약 660일.' }]));
...
body.push(B('간격 30 → 70 m. 1 PV가 46 → 250일. 정호 수는 같은데 체류시간이 5배. Honeymoon이 광역 간격으로 간 논리.'));
body.push(B('이온흡착형 가정 켜기. 정호당 주입 100 → 19 m³/d, 1 PV 약 660일. 같은 기간을 맞추려면 정호를 5배 촘촘히 박아야 합니다. 이 방식이 이온흡착형에 맞지 않는 이유가 숫자로 나옵니다.'));
...
body.push(NUM('참조해가 들어오면 우측 패널에 “참조해 비교” 섹션이 나타납니다. 현재 표시 항목(침출제 또는 수두)의 평면도를 브라우저 · OGS · 차이(브라우저 − OGS, 붉음 = 브라우저가 큼) 세 장으로 나란히 보여 주고, 그 아래에 OGS 프레임 시각, 수두 RMS 차, 추적자 R²를 씁니다. 참조해를 불러온 뒤 설정을 바꾸면 “참조해 시나리오와 현재 설정이 다름” 경고가 어떤 항목이 다른지와 함께 뜹니다.'));
body.push(NUM('“3D 면에 OGS 결과 표시”를 켜면(목록에서 불러오면 자동으로 켜집니다) 3D 평면의 침출제·수두가 브라우저 대신 OGS 결과로 바뀝니다. 브라우저 계산은 그대로 진행됩니다. 상태 상자에 “3D 면 = OGS 참조해” 줄이 나타나고, OGS에 없는 표시 항목(모액·침출 진행도)이나 OGS 데이터 범위 밖에서는 그 사실을 알리고 브라우저 결과를 그립니다. 참조해는 침출 0–5 PV와 복원 5–7 PV를 모두 담고 있습니다.'));
body.push(...IMGP('shots/14_ogs_loaded.png', 600, 360, '그림 7-2. 7-spot · 30 m · bleed −10 % 참조해를 불러와 재생한 화면. 3D 면은 OGS 추적자, 우측 “참조해 비교”에 브라우저 · OGS · 차이 평면도'));
...
body.push(P('패턴 탭을 OpenGeoSys 참조해와 12개 시나리오(3 패턴 × 간격 30·50 m × bleed +5·−10 %, 침출 5 PV + 복원 2 PV)에서 같은 PV 기준으로 비교했습니다. 상세는 COMPARE.md 부록 C에 있습니다.'));
body.push(table(['지표', '브라우저 − OGS', '해석'], [
  ['수두 RMS', '0.12~0.35 m (범위의 1~3 %)', '흐름 풀이는 참조해와 같다'],
  ['sweep 접촉율 @1 PV', '브라우저 −2.4 %p (−5 ~ −1)', '분산항 도입 후. bleed 조건과 무관. 브라우저가 몇 %p 보수적이라고 붙여 읽는다'],
  ['sweep 접촉율 @5 PV', '−0.8 %p', '장기적으로 수렴'],
  ['광체 밖 유출', '±0.6 %p', '봉쇄 성립·실패 모두. 중앙값으로 읽는다'],
  ['파과 시각', '−3 ~ +5일', '운영 일정에는 무시 가능'],
  ['복원 2 PV 뒤 잔류', '±2 %p', '주입 정지 후 양수로 되끌어온 뒤 남는 양'],
], [2400, 2600, 4100]));
body.push(SP());

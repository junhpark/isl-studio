...
문구는 한국어·영어 두 벌이다. 앱 바 우측 **KO / EN** 으로 바꾸고(브라우저에 기억), `?lang=en` 을 붙여 열면 영어로 시작한다.
영어 표는 `manual/i18n_en.json` 에 있고 `python3 manual/i18n_build.py` 가 HTML 에 넣는다(`--dom` 을 주면 빠진 문구를 찾아 준다).

모바일(≤900 px)에서는 좌·우 패널이 하단 **설정 / 지표 / 범례** 버튼으로 여닫는 서랍이 되고,
3D 화면은 한 손가락 드래그로 회전, 두 손가락으로 확대·축소한다.

**편집은 `isl-studio.html`에서만 한다.** 고친 뒤 `python3 manual/i18n_build.py && python3 manual/build_offline.py` 로 영어 표를 넣고 오프라인 판을 다시 만든다. 패턴 물리를 고쳤으면 `python3 ogs1/browser/build_headless.py` 도.
...
ogs1/                      OpenGeoSys Phase 1 키트
  ...
  runner/                  make_case / run_case / fields_io / compare / make_library / make_index / library_summary / make_figures
  runner/templates/        pattern_hc.prj.tmpl (이류형 + 등방확산 안정화)
  browser/                 헤드리스 배치 (Node) — build_headless.py 가 스튜디오의 Pattern 모듈에서 생성
...
manual/                    설명서·기술배경서 생성 스크립트 (docx-js), 수식(eqs.py)·개념도(schematics.py), 스크린샷(shot.py), 영어 표(i18n_en.json + i18n_build.py), 오프라인 판(build_offline.py)
```
...
12개 합계 약 8 MB (침출 5 PV + 복원 2 PV, 원본 float32 약 90 MB). 형식은 `ogs1/runner/fields_io.py` 참고. 원본 `fields.bin`, `*.vtu`, 로그는 제외했다(`.gitignore`).
`ogs1/README.md`의 재현 절차대로 `run_case.py`를 돌리면 다시 생긴다(케이스당 3~12분, 12개 라이브러리 2코어 약 45분).
...
- sweep 접촉율(추적자 > 5 %): 같은 PV 에서 12개 평균 −2.4 %p @1 PV(보수적), −0.8 %p @5 PV — bleed 조건과 무관
- 광체 밖 추적자: ±0.6 %p (봉쇄 성립·실패 모두)
- 파과 시점: −3~+5일 · 복원 2 PV 뒤 잔류: ±2 %p

상세와 정정 기록(참조해 물질수지 결함, 라이브러리 도메인 불일치, PV 기준·7-spot 배치·복원 단계)은 `docs/COMPARE.md`.
